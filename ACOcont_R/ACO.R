...
ACO <- function(f, e, n.of.ants=2, k=50, q=0.0001, xi=0.85, 
  ...
  # holds evals and fct value
  runtimemat = matrix(0, nrow=0, ncol=2)
...
#      p <- rbind(p,c(v=y[i],sd=0,gr=0,lr=0,ld=0))
    ...
    if((eval - 100) > lastevalsample) {
      ...
      runtimemat = rbind(runtimemat, c(eval, bestvalue))
       lastevalsample = eval
     }
    ...
    if (max(y)>max.y) {
      ...
      # AA 25.03.2011, this works but I only want to break on evals
      #if ((abs(max.y-max.value)<abs(e.rel*max.value+e.abs)) |
      #				(max.y>max.value))
          #return(c(as.integer(eval)))
      #    return(runtimemat)
    }

    if (max.eval>0 & eval>=max.eval) {
      #return(c(as.integer(eval)))
      runtimemat = rbind(runtimemat, c((eval-1), abs(max.y) ))
      return(runtimemat)
    }
  }
