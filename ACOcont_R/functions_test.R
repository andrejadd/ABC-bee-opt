...
sc <- c(f=sc.f,d=list(sc.d),tf=sc.tf,opt=0)
...
rg <- c(f=rg.f,d=list(rg.d),tf=rg.tf,opt=0)
...
ac.f <- function(u,o,x) {
	...
	l_c <- rep(0,dim(x)[1])
	for (j in 1:d) {
    	    l_c <- l_c + cos(2* pi* x[,j])
  	}
	l_c <- exp(l_c / d)
	
	l_d <- -20 * l_b - l_c + 20 + exp(1)

	return(-l_d)
}

ac <- c(f=ac.f,d=list(ac.d),tf=ac.tf,opt=0)
