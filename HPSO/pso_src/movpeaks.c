...
double width_severity = 0.6; /* severity of width changes, larger numbers
                               mean larger severity 0.01*/
...
int use_basis_function=1; /* if set to 1, a static landscape (basis_function) is included in the fitness evaluation */
...
/* width chosen randomly when standardwidth = 0.0 */
//double minwidth = 0.8, maxwidth = 7.0, standardwidth = 0.0; 
double minwidth = 1.0, maxwidth = 12.0, standardwidth = 0.0; 
...
void set_geno_size(int dim) {
	geno_size = dim;	
}

void set_number_peaks(int peaks) {
	number_of_peaks = peaks;	
}

void set_vlength(double severity) {
	vlength = severity;
}
