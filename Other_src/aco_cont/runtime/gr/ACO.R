...
ACO <- function(f, e, n.of.ants=2, k=50, q=0.0001, xi=0.85, 
  max.eval=0, seed=0, ls=NULL, outfile) {
...
# AA-RETURN -> write -1 if converged 
    if (sum(apply(dist.mean,2,sd))==0) {   # population has converged!
         outstring <- paste(as.integer(eval), -max.y, "-1" , sep="\t")   
         
	 write(outstring,file=outfile, append=TRUE)
	
	return(as.integer(max.eval))  
...
#      p <- rbind(p,c(v=y[i],sd=0,gr=0,lr=0,ld=0))
    ...
    for (i in 1:k) 
     ...
     if(max(y) == 0.0) {
           outstring <- paste(as.integer(eval), -max(y), sep="\t")   
      	   write(outstring,file=outfile, append=TRUE)
     	   return(as.integer(eval))    
     }
...
#     do not return optima
#      if ((abs(max.y-max.value)<abs(e.rel*max.value+e.abs)) |(max.y>max.value))
#	return(as.integer(eval))          
