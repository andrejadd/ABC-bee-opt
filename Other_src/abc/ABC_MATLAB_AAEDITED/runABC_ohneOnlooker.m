...
% Diese Datei hat keine Onlooker Bees, trotzdem ist ColonySize gleich (neb*2). Also um Bsp. eine Colony mit 50 Employed Bees zu haben muss ColonySize 100 gesetzt werden
 ...
 ABCOpts = struct( 'ColonySize',  10, ...   % Number of Employed Bees+ Number of Onlooker Bees 
    ...
    'Limit',   225, ... % Control paramter in order to abandone the food source 
...
% (Anm: AA: Bas is an integer array holding the abandon count for each solution)
...
Evals = Cycle * (ABCOpts.ColonySize/2);
%fprintf('Cycle=%d , Evals=%d , ObjVal=%g\n',Cycle, Evals,GlobalMin);

 if(GlobalMin < ABCOpts.ErrGoal )
   fprintf('Goal Reached! Evals=%d\n', Evals);
   break;
  end;
