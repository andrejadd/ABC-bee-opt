...
public class ABCsel_gbest_3a3 implements ModelInterface {
	...
	private ArrayList<Solution> gC_gbest = new ArrayList<Solution>();
	...
	private int gi_gbest = 3;
	
	public ABCsel_gbest_3a3() {}
	...
	private void UpdateGBest() {
		...
		if(gC_gbest.size() == 0) {
			for(int i = 0; i < gi_gbest; i++) {
				...
				gC_gbest.add(new Solution(lC_tmp.getPosition(), lC_tmp.value));
			}
			...
			for(int i = 0; i < gi_gbest; i++) {
				Solution lC_tmp = (Solution) lCa_sortedSolutions[i];
				Solution lC_gbest = (Solution) gC_gbest.get(i);
				
				if(lC_tmp.value != lC_gbest.value) {
					gC_gbest.set(i, (Solution) lC_tmp.clone());
				}
	...
	private double[] modifyposition(Solution aC_solution, int ai_index) {
		...
		Solution lC_refgbest = gC_gbest.get(generator.nextInt(gi_gbest));
