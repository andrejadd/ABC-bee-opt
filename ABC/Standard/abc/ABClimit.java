...
public class ABClimit implements ModelInterface {
	...
	private double gd_limitscale = 0.1;
	...
	public void Init(TestFunction am_fcttest, ParameterHolder aC_params) {
		...
		gi_limit = (int) (gd_limitscale * (gi_n * gi_dim));
		
		if(aC_params.containsKey(GlobalParameters.LIMIT)) {
			gi_limit = (Integer) aC_params.get(GlobalParameters.LIMIT);
		}
		...
		if(!lb_optimareached) {
			...
			/* 4: Calculate Prob. values P_i */
			 ...
			 for(int i = 0; i < gi_n; i++) {
				...
				if((lC_sol.limit >= gi_limit) && (gi_limit != 0)) {
