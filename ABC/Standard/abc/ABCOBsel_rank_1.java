...
public class ABCOBsel_rank_1 implements ModelInterface {
	...
	public void calcSelectProbs(){
		...
		int li_sumsize = (li_rank * (li_rank+1)) / 2;
			  
		for(int i =0; i < lCa_sortedSolutions.length; i++) {
			Solution lC_sol = (Solution) lCa_sortedSolutions[i];
			lC_sol.rank = li_rank--;
			lC_sol.select_prob = (double)lC_sol.rank / (double) li_sumsize;
		}
