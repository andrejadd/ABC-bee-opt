...
public class ABCgbestdist_1 implements ModelInterface {
...
private double[] modifyposition(Solution aC_solution, int ai_index) {
		...
		double ld_phi = (generator.nextDouble() * 2) - 1;
		double ld_gamma = generator.nextDouble() -1;
		
		int j = generator.nextInt(gi_dim);
		
		double ld_rand = ld_phi * (position[j] - lda_refpos[j]); 
...
public int selectDist() {
	...
	for(int i = 0; i < gi_n; i++) {
	    ...
	    if(tmpsol.dist_prob == 0.0) {
	    	continue; // no prob. is the own solution 
	    }
	    
	    	if( (rand > sum) && (rand <= (sum + tmpsol.dist_prob))) {
	    		
	    		return i;  // the entry selected from the BB, needed later to make update in bb
	    		
			}
	    
	    sum += tmpsol.dist_prob;
	}
	
	return -1;
}

	/* calculate the selection prob. for solutions around solution  with index ai_index */ 
	private void calcSelectProbDist(int ai_index) {
		...
		for(int i = 0; i < gi_n; i++) {
			...
			double d = getDistance(ai_index, i);
			
			if(d!=0) { /* no devision by zero */
				sum += ( 1 / d);
			}
		}
		...
		for(int i = 0; i < gi_n; i++) {
			...
			if(i == ai_index) {
				lC_tmp.dist_prob = 0.0;
			}
			
			double ld_thisd = getDistance(ai_index, i);
			if(ld_thisd == 0.0) {
				lC_tmp.dist_prob = 0.0;
			} else {
				double h =  (1/ ld_thisd) / sum;
				lC_tmp.dist_prob = h;
			}
		}
