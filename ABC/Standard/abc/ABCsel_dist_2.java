...
public class ABCsel_dist_2 implements ModelInterface {
...
public double dist(double[] ai_c1, double[] ai_c2) {
	...
	private void calcRankProbs(int ai_index) {
		...
		int li_ranksum = ((gi_n - 1) * gi_n) / 2; /* add up all ranks */
		...
		for(int i = 0; i < gi_n; i++) {
			...
			double p = (double) li_rank / li_ranksum;
			lC_tmp.rank_prob = p;
			li_rank--;
		}
