...
public class ABCphi_4a implements ModelInterface {
	...
	private double gd_maxscale = 1.0;
	//private double gd_phistep;
	
	
	/* remembered Max and Min values */
	private double gd_minvalue;
	private double gd_maxvalue;
	...
	public void Init(TestFunction am_fcttest, 
		...
		gi_n = (Integer)aC_params.get(GlobalParameters.NEB); /* assume nr. EB and nr. OB are the same */
		...
		Solution lC_best = (Solution)  lCa_sortedSolutions[0];
		Solution lC_worsed = (Solution)  lCa_sortedSolutions[lCa_sortedSolutions.length - 1];
		
		gd_minvalue = lC_best.value;
		gd_maxvalue = lC_worsed.value;
	...
	private double[] modifyposition(Solution aC_solution, int ai_index) {
		...
		double phi_scaler = getPhiDependendSolutionQuality(ai_index);
		
		double phi_ij = (generator.nextDouble() * (2*phi_scaler)) - phi_scaler;
	...
	private double getPhiDependendSolutionQuality(int ai_index) {
		...
		if(lC_best.value < gd_minvalue) {
			gd_minvalue = lC_best.value;
		}

		if(lC_worsed.value > gd_maxvalue) {
			gd_maxvalue = lC_worsed.value;
		}

		
		Solution lC_current = (Solution) gC_solutions.get(ai_index);
		
		/* as better the value , as smaller the  normvalue */
		double ld_normvalue = (lC_current.value - gd_minvalue) / (gd_maxvalue - gd_minvalue);
		
		double ld_rangediff = Math.abs(gd_maxscale - gd_minscale);
		
		double ld_phi = gd_minscale + (ld_rangediff * ld_normvalue);
