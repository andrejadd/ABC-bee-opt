...
public class ABCphi_4b implements ModelInterface {
	...
	public void Init(TestFunction am_fcttest, 
		...
		java.util.Arrays.sort(lCa_sortedSolutions, new Value_Comparator()); 
	...
	private double[] modifyposition(Solution aC_solution, int ai_index) {
		...
		double phi_scaler = getPhiDependendSolutionRank(ai_index);
...
private double getPhiDependendSolutionRank(int ai_index) {
		...
		double ld_normvalue = (ld_maxrank - lC_current.rank) / (ld_maxrank - ld_minrank);
