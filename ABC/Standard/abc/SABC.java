...
public class SABC implements ModelInterface {
...
//		gi_limit = ((gi_neb+gi_nob)/2) * gi_dim;	
		
		if(! (Boolean) aC_params.get(GlobalParameters.STANDARDLIMIT)) {
			if(gi_limit < 100) {
				gi_limit = 100;
			}
		}
	...
	private double[] modifyposition(Solution aC_solution, int ai_index) {
		...
		do {
			...
			/* sample the reference solution */
			k = generator.nextInt(gi_neb);
			
			/* sample the parameter to modify */
			j = generator.nextInt(gi_dim);
			
	    } while (ai_index == k);  /* as long as this solution is not reference solution */ 
		
		/* get reference solution */
		Solution lC_refsol = gC_solutions.get(k);
		double [] lda_refpos = lC_refsol.getPosition();
		
		/* sample uniform random in [-1;1] */
