...
public class ABCphi_2 implements ModelInterface {
	...
	/* scaler */
	private double gd_minscale = 0.5;
	private double gd_maxscale = 1;
	private double gd_phistep;
	
	public ABCphi_2() {}
	
	public void Init(TestFunction am_fcttest, ParameterHolder aC_params) {
		...
		// test: decrease for first 200 iterations
		gd_phistep = Math.abs(gd_maxscale - gd_minscale) / 400;
		//(Integer) aC_params.get(GlobalParameters.ITERATIONS));
	}
	...
	private double[] modifyposition(Solution aC_solution, int ai_index) {
		...
		double li_scaler = gd_maxscale - (gd_phistep * gi_cycle);
		if(li_scaler < gd_minscale) {
			li_scaler = gd_minscale;
		}
						
		double phi_ij = (generator.nextDouble() * (2*li_scaler)) - li_scaler;
