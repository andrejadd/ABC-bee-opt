...
public class ABCgbestrank_2 implements ModelInterface {
	...
	private void setRankProbs(int ai_rank) {
		...
		for(int i = 0; i < gi_n; i++) {
			...
			double li_rd = Math.abs(lC_tmp.rank - ai_rank);
			sum += (1/ li_rd);
		}
		double tsum = 0;
		for(int i = 0; i < gi_n; i++) {
			...
			double li_rd = (1 / (double)Math.abs(lC_tmp.rank - ai_rank)) / sum;
			lC_tmp.rank_prob = li_rd;
			tsum += li_rd;
		}
