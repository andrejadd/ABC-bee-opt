...
public class ABCsel_rank_3 implements ModelInterface {
	...
	private double[] modifyposition(Solution aC_solution, int ai_index) {
		...
		try {
			...
			String lC_emsg = e.getMessage();
			System.out.println(lC_emsg);
			System.exit(-1);
		}
		...
		/* make sure not to select the same rank, not clean but works */
		if( k == li_ownrank) {
			if(generator.nextBoolean() && (k < li_maxrank)) {
				k+=1; /* shift one to the right */
			} else if (k > 1){  // k greater than min(ranks)
				k-= 1;
			} 
		}
