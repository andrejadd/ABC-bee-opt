...
public class ABCdim_1 implements ModelInterface {
	...
	private double[] modifyposition(Solution aC_solution, int ai_index) {
		...
		for (int i = 0; i < position.length; i++) {
			...
			if((generator.nextDouble() < gd_CR ) || (j == i)) {
				
				newposition[i] = position[i] + phi_ij * (position[i] - lda_refpos[i]);
				newposition[i] = fct1.checkrange(newposition[i]);
			} 
		}
