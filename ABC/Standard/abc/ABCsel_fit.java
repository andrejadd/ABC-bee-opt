...
public class ABCsel_fit implements ModelInterface {
	...
	private double[] modifyposition(Solution aC_solution, int ai_index) {
		...
		try{	
			...
			System.out.println("error retrieving solution k: " + k + " Message: " + ie.getMessage());
			System.exit(-1);
		};
	...
	/* This is the actual mod: Probability selection of reference according to solution quality */
	public int select() {
	   		
			double rand = generator.nextDouble();
			double sum = 0;
		
			/* Select a solution from the blackboard via roulette wheel selection, select probs already exist from the OB selection procedure */
			for(int i = 0; i < gi_n; i++) {
