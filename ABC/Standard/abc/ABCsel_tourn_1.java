...
public class ABCsel_tourn_1 implements ModelInterface {
	...
	private int gi_tournament_rounds = 0; 
	...
	public void Init(TestFunction am_fcttest, 
		...
		gi_tournament_rounds = (Integer)aC_params.get(GlobalParameters.TOURNAMENT_ROUNDS);		
	...
	public void calcSelectProbs(){
	  ...
	  public int select() {
		...
		private void MakeTournament() {
			...
			/* reset tournament ranks */
			for(int i =0; i < gi_n; i++) {
				Solution tmp = (Solution) gC_solutions.get(i);
				tmp.rank=0;
			}
			
			if (gi_tournament_rounds == 0) {
					throw new NullPointerException();
			
			}
			for(int i =0; i < gi_tournament_rounds; i++) {
				
				int li_index1 = generator.nextInt(gi_n);
				int li_index2 = -1;
				do {
						li_index2 = generator.nextInt(gi_n);
				} while (li_index1 == li_index2);
			
				double ld_value1 = gC_solutions.get(li_index1).value;
				double ld_value2 = gC_solutions.get(li_index2).value;
				
					if(fct1.better(ld_value1,ld_value2)) {
						Solution tmp = (Solution) gC_solutions.get(li_index1);
						tmp.rank++;
					} else if (fct1.better(ld_value2,ld_value1)) {
						Solution tmp = (Solution) gC_solutions.get(li_index2);
						tmp.rank++;
					}
				}
				/* this calculates rank_prob */
				calcRankProb();
		 
		}
		
		
		  private void calcRankProb() {
			...
			for(int i = 0; i < gi_n; i++) {
			    ...
			    /* sets value of reference to the solution */ 
			    lC_tmpsol.rank_prob = lC_tmpsol.rank / sum_solution;
			   
			}
			
	   }
		  
		  public int selectWithRank() {
				...
				for(int i = 0; i < gi_n; i++) {
				    Solution tmpsol = (Solution) gC_solutions.get(i);
				    	...
				    	if( (rand > sum) && (rand <= (sum + tmpsol.rank_prob))) {
				    		
				    		return i;  // the entry selected from the BB, needed later to make update in bb
				    		
						}
				    
				    sum += tmpsol.rank_prob;
				}
